...
export async function GET(request: NextRequest) {
  ...
  const [rows, total] = await Promise.all([
    prisma.userPhraseProgress.findMany({
      ...
      skip: offset,
      take: limit,
      include: { phrase: { select: { spanish: true, english: true } } },
    }),
    prisma.userPhraseProgress.count({ where }),
  ])

  const items = rows.map(r => ({
    spanish: r.phrase.spanish,
    english: r.phrase.english,
    score: r.score,
  }))
